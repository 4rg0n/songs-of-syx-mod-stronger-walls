...
public class TFortification extends TerrainTile implements TDestoryable, INDEXED, Diagonalizer{
	...
	TFortification(String key, Terrain t, LISTE<TFortification> all, Json data, Json text, SPRITE icon, TILE_SHEET sheet) {
		...
		this.sheet = sheet;
		// MODDED: the max value was increased from 1 to 100
		durability = data.d("DURABILITY", 0, 100.0)*C.TILE_SIZE;
		desc = text.text("DESC");
		this.key = key;
...
//		}
	...
	protected boolean renderBelow(SPRITE_RENDERER r, ShadowBatch s, RenderIterator i, int data) {
		...
		if (code != 0x0F && ((GAME.updateI() + i.tx()) & 0x0FF) == 0) {
			Floors.setFloorMatch(i.tx(), i.ty(), null);
		}
		
		return false;
	}
	
	private void renderEdges(SPRITE_RENDERER r, ShadowBatch s, RenderIterator i, int data) {
		int cor = (data >> 4) & 0x0F;
